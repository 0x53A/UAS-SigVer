...
impl eframe::App for AliasApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            let plot_height = ui.available_height() / 4.0 - 10.0; // 4 plots with spacing
            let plot_width = ui.available_width();
            
            // Helper function to draw axis labels
            let draw_axis_labels = |painter: &egui::Painter, rect: egui::Rect, x_label: &str, y_label: &str| {
                // X-axis label
                painter.text(
            ...
            ui.colored_label(Color32::YELLOW, format!("Original signal ({}Hz) with sample points ({}Hz)", 
            ...
            if let rect = response1.rect.intersect(ui.clip_rect()) {
                ...
                for i in 0..signal.len() - 1 {
                    ...
                    painter.line_segment(
                        [
                            rect.left_top() + vec2(x1 / (2.0 * PI) * rect.width(), rect.height() / 2.0 - y1 * (rect.height() / 2.0)),
                            rect.left_top() + vec2(x2 / (2.0 * PI) * rect.width(), rect.height() / 2.0 - y2 * (rect.height() / 2.0)),
                        ],
                ...
                for (x, y) in &sample_points {
                    painter.circle_filled(
                        rect.left_top() + vec2(*x / (2.0 * PI) * rect.width(), rect.height() / 2.0 - y * (rect.height() / 2.0)),
                        4.0,
                        Color32::RED,
                    );
                }
            ...
            ui.colored_label(Color32::YELLOW, format!("Sample points only (Sampling rate: {}Hz)", self.sampling_frequency));
            ...
            ui.colored_label(Color32::YELLOW, "FFT of sampled points (Frequency spectrum)");
            ...
            if let rect = response3.rect.intersect(ui.clip_rect()) {
                ...
                let fft_size = 8192; // Doubled for better resolution
                
                // Create much longer signal for better frequency resolution
                // We'll simulate many more cycles than what we display
                let extended_signal_len = 10000; // 10x more samples than before
                let mut extended_signal = Vec::with_capacity(extended_signal_len);
                
                // Generate many more cycles of the signal for better FFT
                // This simulates sampling a continuous signal for much longer
                let cycles_to_simulate = 50.0; // Simulate 50 cycles instead of just a few
                
                for i in 0..extended_signal_len {
                    // Scale time to cover more cycles but maintain correct sampling rate
                    let t = i as f32 / self.sampling_frequency;
                    if t * self.signal_frequency < cycles_to_simulate { // Many more cycles
                ...
                // Apply a better window function to reduce spectral leakage
                // Blackman window provides better side lobe suppression than Hann
            ...
            ui.colored_label(Color32::YELLOW, format!("Reconstructed signal ({}Hz sampling)", self.sampling_frequency));
